 ...
 */
function rootFlare(pts, r, k = 1) {
  const p0 = pts[0].p;
  const r0 = pts[0].r;
  pts.unshift({ p: new THREE.Vector3(p0.x, p0.y - 0.45, p0.z), r: r0 * (2.05 + r() * 0.5) * k });
  pts.splice(1, 0, { p: new THREE.Vector3(p0.x, p0.y + 0.02, p0.z), r: r0 * (1.42 + r() * 0.3) * k });
  if (pts[2]) pts[2].r *= 1 + 0.12 * k;
  return pts;
}
...
/** Maple cards: mostly green, a slice of autumn orange-red (#C45A28 / #7A8A3A). */
function mapleTint(r) {
  const v = 0.82 + r() * 0.30;
  if (r() < 0.38) {
    return [v * (1.24 + r() * 0.18), v * (0.46 + r() * 0.22), v * (0.20 + r() * 0.12)];
  }
  return [v * (0.72 + r() * 0.18), v * (0.96 + r() * 0.12), v * (0.40 + r() * 0.16)];
}
...
export const SPECIES_INFO = {
  /* Near-black bole (#1A1612) against gold grass — a thin multiplier on already
     dark pine bark, not a mid-tone wash. */
  pine: { bark: 'bark_pine', tint: [0.48, 0.40, 0.32], barkRough: 0.97 },
  cottonwood: { bark: 'bark_oak', tint: [0.90, 0.78, 0.64], barkRough: 0.90 },
  scrubOak: { bark: 'bark_oak', tint: [0.88, 0.80, 0.66], barkRough: 0.91 },
  snag: { bark: 'bark_birch', tint: [0.78, 0.76, 0.70], barkRough: 0.88 },
};
