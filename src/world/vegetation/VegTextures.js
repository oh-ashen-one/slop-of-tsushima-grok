...
function drawBlade(g, x0, y0, len, wid, lean, hue, sat, li, r, seedProb = 0.16) {
  ...
  grd.addColorStop(0, hsl(hue + 22, sat * 0.72, li * 0.36));
  grd.addColorStop(0.32, hsl(hue, sat * 1.05, li * 0.82));
  grd.addColorStop(0.70, hsl(hue - 2, sat * 0.72, Math.min(0.78, li * 1.16)));
  grd.addColorStop(1, hsl(hue - 4, sat * 0.26, Math.min(0.90, li * 1.52)));
  g.fillStyle = grd;
  g.beginPath();
  ...
  if (r() < seedProb) {
    g.fillStyle = hsl(42, 0.16, 0.84, 0.92);
    for (let i = 0; i < 7; i++) {
      const t = 0.72 + i * 0.04;
      const mt = 1 - t;
      const px = mt * mt * x0 + 2 * mt * t * cx + t * t * tipX;
...
function drawSeedHead(g, x0, y0, len, lean, hue, sat, li, r) {
  ...
  g.quadraticCurveTo(cx, cy, tipX, tipY);
  g.stroke();
  // spikelets fanning off the top third — cream pampas tips, not dusty straw
  const n = 11 + ((r() * 8) | 0);
  for (let i = 0; i < n; i++) {
    const t = 0.58 + 0.42 * (i / n);
    const [px, py] = at(t);
    const side = i % 2 === 0 ? 1 : -1;
    const sl = len * (0.11 + 0.16 * r()) * (1 - (t - 0.58) * 0.7);
    const ang = side * (0.50 + r() * 0.60) + lean * 0.6;
    g.strokeStyle = hsl(40 + r() * 8, 0.14 + r() * 0.10,
      Math.min(0.90, 0.78 + r() * 0.12), 0.88);
    g.lineWidth = Math.max(0.7, len * 0.014);
    g.beginPath();
    g.moveTo(px, py);
...
/**
 * Feathery cream plume — Iki Island pampas. Overlapping pale ellipses on a
 * gold culm, read as a seed head at ten metres and as a bright rim at 30.
 */
function drawPampasHead(g, x0, y0, len, lean, r) {
  const tipX = x0 + lean * len * 1.18;
  const tipY = y0 - len;
  const cx = x0 + lean * len * 0.22;
  const cy = y0 - len * 0.62;
  const at = (t) => {
    const mt = 1 - t;
    return [mt * mt * x0 + 2 * mt * t * cx + t * t * tipX,
  ...
  g.strokeStyle = hsl(46, 0.30, 0.42);
  g.lineWidth = Math.max(0.8, len * 0.010);
  g.beginPath();
  g.moveTo(x0, y0);
  g.quadraticCurveTo(cx, cy, tipX, tipY);
  g.stroke();
  const n = 24 + ((r() * 14) | 0);
  for (let i = 0; i < n; i++) {
    const t = 0.50 + 0.50 * (i / n);
    const [px, py] = at(t);
    const side = i % 2 === 0 ? 1 : -1;
    const plume = len * (0.14 + 0.22 * r()) * (0.50 + (t - 0.50) * 1.15);
    const ang = side * (0.32 + r() * 0.90) + lean * 0.45;
    const ex = px + Math.sin(ang) * plume;
    const ey = py - Math.cos(ang) * plume * 0.58;
    g.fillStyle = hsl(40 + r() * 8, 0.10 + r() * 0.12, 0.80 + r() * 0.12, 0.82);
    g.beginPath();
    g.ellipse((px + ex) * 0.5, (py + ey) * 0.5,
      plume * 0.22, plume * 0.52, ang, 0, 6.2832);
    g.fill();
  }
}
...
function drawFlower(g, x0, y0, len, lean, kind, fh, fs, fl, r) {
  ...
  g.strokeStyle = hsl(62, 0.28, 0.34);
...
/**
 * WILDFLOWER TILES — eight white / cream stalks, one per atlas column, drawn
 * as a whole plant (roots at the bottom of the cell, head at the top) so a
 * single card is a plant rather than a decal. Iki Island specks, not western.
 *
 * THE COLOUR RULE. Iki Island wildflowers are white / cream specks (#F6F1E4)
 * drifting through gold grass, not western paintbrush. Petals sit at 0.82–0.90
 * lightness so they read in the near 20 m; chroma stays low so they do not
 * blow out at golden hour. Contrast comes from cream against gold, not hue.
 */
...
/* 0 — Cream spike. Dense pale bracts, a white plume in gold grass. */
function flPaintbrush(g, T, r) {
  basalLeaves(g, T, r, { hue: 62, sat: 0.22, li: 0.36, n: 7, len: 0.19 });
  const stems = 4 + ((r() * 3) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.22 + 0.56 * ((i + 0.5) / stems + (r() - 0.5) * 0.18));
    const len = T * (0.50 + r() * 0.26);
    const lean = (r() - 0.5) * 0.28;
    const [tx, ty] = drawStem(g, x, T * 0.99, len, lean, T * 0.014, 58, 0.24, 0.32);
    const n = 9 + ((r() * 5) | 0);
    for (let k = 0; k < n; k++) {
      const t = k / n;
      ...
      petal(g, px, py, rr * 0.60, rr, (r() - 0.5) * 1.1,
        42 + r() * 8, 0.10 + r() * 0.08, 0.80 + r() * 0.10);
    }
    petal(g, tx, ty + T * 0.008, T * 0.023, T * 0.035, 0, 44, 0.12, 0.88);
  }
}

/* 1 — White daisy. Long stems, cream rays, gold eye. */
function flMarigold(g, T, r) {
  basalLeaves(g, T, r, { hue: 62, sat: 0.20, li: 0.38, n: 8, len: 0.18 });
  const stems = 4 + ((r() * 3) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.20 + 0.60 * ((i + 0.5) / stems + (r() - 0.5) * 0.20));
    const len = T * (0.56 + r() * 0.30);
    const lean = (r() - 0.5) * 0.34;
    const [tx, ty] = drawStem(g, x, T * 0.99, len, lean, T * 0.011, 58, 0.22, 0.34);
    daisy(g, tx, ty, T * (0.064 + r() * 0.024), 11,
      44 + r() * 6, 0.08, 0.86 + r() * 0.06, 44, 0.28, 0.58, r);
  }
}

/* 2 — White coneflower. Cream rays around a warm gold cone. */
function flConeflower(g, T, r) {
  basalLeaves(g, T, r, { hue: 60, sat: 0.22, li: 0.36, n: 6, len: 0.21 });
  const stems = 3 + ((r() * 2) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.24 + 0.52 * ((i + 0.5) / stems + (r() - 0.5) * 0.20));
    const len = T * (0.66 + r() * 0.26);
    const lean = (r() - 0.5) * 0.22;
    const [tx, ty] = drawStem(g, x, T * 0.99, len, lean, T * 0.011, 56, 0.24, 0.34);
    const ch = T * (0.064 + r() * 0.024);
    g.fillStyle = hsl(40, 0.28, 0.42);
    g.beginPath();
    ...
    for (let k = 0; k < petals; k++) {
      ...
      petal(g, tx + Math.cos(a) * T * 0.032, ty + T * 0.036,
        ...
        44 + r() * 6, 0.08, 0.84 + r() * 0.08);
    }
  }
}

/* 3 — Cream cups. Four-petal white-cream over gold-green leaves. */
function flPrimrose(g, T, r) {
  basalLeaves(g, T, r, { hue: 64, sat: 0.24, li: 0.38, n: 9, len: 0.27 });
  const heads = 4 + ((r() * 3) | 0);
  for (let i = 0; i < heads; i++) {
    const x = T * (0.20 + 0.60 * ((i + 0.5) / heads + (r() - 0.5) * 0.22));
    const len = T * (0.32 + r() * 0.28);
    const [tx, ty] = drawStem(g, x, T * 0.99, len, (r() - 0.5) * 0.40, T * 0.012, 58, 0.24, 0.34);
    const R = T * (0.078 + r() * 0.026);
    for (let k = 0; k < 4; k++) {
      const a = (k / 4) * 6.2832 + r() * 0.25;
      petal(g, tx + Math.cos(a) * R * 0.42, ty + Math.sin(a) * R * 0.30,
        R * 0.40, R * 0.33, a, 44 + r() * 6, 0.10 + r() * 0.06, 0.84 + r() * 0.08);
    }
    g.fillStyle = hsl(46, 0.32, 0.62);
    g.beginPath();
...
/* 4 — White umbel. A bright cream plate on a gold stem — the field_16 speck. */
function flYarrow(g, T, r) {
  for (let i = 0; i < 10; i++) {
    drawSprig(g, T * (0.22 + 0.56 * r()), T * 0.99, T * (0.15 + r() * 0.16),
      (r() - 0.5) * 0.7, 64, 0.22, 0.38, r);
  }
  const stems = 5 + ((r() * 3) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.20 + 0.60 * ((i + 0.5) / stems + (r() - 0.5) * 0.18));
    const len = T * (0.54 + r() * 0.28);
    const [tx, ty] = drawStem(g, x, T * 0.99, len, (r() - 0.5) * 0.16, T * 0.011, 58, 0.22, 0.36);
    const R = T * (0.078 + r() * 0.026);
    for (let k = 0; k < 18; k++) {
      const a = r() * 6.2832, rr = Math.sqrt(r()) * R;
      petal(g, tx + Math.cos(a) * rr, ty + Math.sin(a) * rr * 0.34,
        R * 0.22, R * 0.15, 0, 42 + r() * 8, 0.06 + r() * 0.06, 0.84 + r() * 0.08);
    }
  }
}

/* 5 — Cream raceme. Vertical paired blooms, pale instead of violet. */
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let k = 0; k < 6; k++) {
      ...
      drawForbLeaf(g, bx, T * 0.94, ln, ln * 0.20,
        (k / 6 - 0.5) * 2.4 + (r() - 0.5) * 0.3, 62, 0.22, 0.36);
    }
  }
  const stems = 3 + ((r() * 2) | 0);
  for (let i = 0; i < stems; i++) {
    ...
    const [tx, ty] = drawStem(g, x, T * 0.99, len, lean, T * 0.013, 58, 0.24, 0.34);
    const n = 14 + ((r() * 6) | 0);
    for (let k = 0; k < n; k++) {
      const t = k / n;
      ...
      for (let s = -1; s <= 1; s += 2) {
        petal(g, tx - lean * len * 0.42 * t + s * w * 0.42 + (r() - 0.5) * T * 0.008, py,
          ...
          42 + r() * 8, 0.08 + r() * 0.08, 0.82 + r() * 0.10);
      }
    }
  }
}

/* 6 — Cream cups along an arching stem. */
function flMallow(g, T, r) {
  basalLeaves(g, T, r, { hue: 58, sat: 0.20, li: 0.38, n: 7, len: 0.19 });
  const stems = 3 + ((r() * 3) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.22 + 0.56 * ((i + 0.5) / stems + (r() - 0.5) * 0.20));
    const len = T * (0.52 + r() * 0.28);
    const lean = (r() - 0.5) * 0.50;
    const [tx, ty] = drawStem(g, x, T * 0.99, len, lean, T * 0.013, 56, 0.22, 0.34);
    const n = 5 + ((r() * 3) | 0);
    for (let k = 0; k < n; k++) {
      const t = k / n;
      ...
      for (let p = 0; p < 5; p++) {
        ...
        petal(g, px + Math.cos(a) * R * 0.42, py + Math.sin(a) * R * 0.32,
          R * 0.42, R * 0.34, a, 42 + r() * 8, 0.10 + r() * 0.08, 0.82 + r() * 0.10);
      }
    }
  }
}

/* 7 — White aster. Tiny cream rays; the specks between the bigger drifts. */
function flAster(g, T, r) {
  basalLeaves(g, T, r, { hue: 62, sat: 0.22, li: 0.36, n: 8, len: 0.21 });
  const stems = 7 + ((r() * 4) | 0);
  for (let i = 0; i < stems; i++) {
    const x = T * (0.16 + 0.68 * r());
    const len = T * (0.36 + r() * 0.34);
    const [tx, ty] = drawStem(g, x, T * 0.99, len, (r() - 0.5) * 0.40, T * 0.009, 58, 0.22, 0.36);
    daisy(g, tx, ty, T * (0.040 + r() * 0.016), 9,
      42 + r() * 8, 0.08 + r() * 0.06, 0.86 + r() * 0.06, 46, 0.28, 0.60, r);
  }
}
 ...
 *
 *   0 TALL  bunch grass — gold culms, cream pampas heads on two variants
 *   1 TURF  fine short turf — many thin blades, yellow-green understorey
 *   2 STRAW dry gold / pampas — pale, upright, cream seed heads
 *   3 FORB  low ground cover — gold-green rosettes and white wildflower cards
 *
...
export function buildGrassAtlas(seed, tile = 224, aniso = 16) {
  ...
  const bunch = (o) => (gg, r) => {
    ...
    for (let i = 0; i < (o.heads || 0); i++) {
      const x = T * (0.16 + 0.68 * r());
      const hlen = T * o.len * (0.86 + r() * 0.30);
      const hlean = (r() - 0.5) * 0.55;
      if (o.pampas) drawPampasHead(gg, x, T, hlen, hlean, r);
      else drawSeedHead(gg, x, T, hlen, hlean, o.hue + 4, o.sat * 0.45,
        Math.min(0.86, o.li * 1.40), r);
    }
  };
  ...
  const rosette = (o) => (gg, r) => {
    ...
    for (let i = 0; i < 10; i++) {
      drawBlade(gg, T * (0.08 + 0.84 * r()), T, T * (0.32 + r() * 0.42),
        T * 0.012 * (0.7 + r()), (r() - 0.5) * 0.9, 50 + r() * 12,
        0.32, 0.48 + r() * 0.12, r, 0.04);
    }
  };

  const recipes = [
    /* ---- ROW 0 : TALL GOLD BUNCH GRASS -------------------------------- */
    bunch({ n: 18, hue: 48, sat: 0.42, li: 0.52, len: 0.94, w: 0.018, seed: 0.16 }),
    bunch({ n: 20, hue: 46, sat: 0.40, li: 0.54, len: 0.90, w: 0.017, seed: 0.20 }),
    bunch({ n: 14, hue: 50, sat: 0.38, li: 0.50, len: 0.96, w: 0.019, seed: 0.08, heads: 4, pampas: true }),
    bunch({ n: 15, hue: 44, sat: 0.44, li: 0.56, len: 0.92, w: 0.018, seed: 0.08, heads: 5, pampas: true }),
    /* ---- ROW 1 : FINE YELLOW-GREEN TURF ------------------------------- */
    bunch({ n: 32, hue: 62, sat: 0.36, li: 0.44, len: 0.86, w: 0.0120, lean: 1.0, seed: 0.02 }),
    bunch({ n: 28, hue: 66, sat: 0.34, li: 0.42, len: 0.90, w: 0.0130, lean: 1.1, seed: 0.02 }),
    bunch({ n: 36, hue: 58, sat: 0.38, li: 0.46, len: 0.82, w: 0.0115, lean: 1.2, seed: 0.0 }),
    bunch({ n: 26, hue: 70, sat: 0.32, li: 0.40, len: 0.92, w: 0.0140, lean: 0.9, seed: 0.03 }),
    /* ---- ROW 2 : DRY GOLD / PAMPAS ------------------------------------ */
    bunch({ n: 13, hue: 44, sat: 0.36, li: 0.62, len: 0.88, w: 0.021, lean: 0.42, seed: 0.18, heads: 3, pampas: true }),
    bunch({ n: 15, hue: 42, sat: 0.38, li: 0.60, len: 0.80, w: 0.019, lean: 0.55, seed: 0.24, heads: 4, pampas: true }),
    bunch({ n: 11, hue: 48, sat: 0.32, li: 0.66, len: 0.94, w: 0.023, lean: 0.34, seed: 0.12, heads: 3, pampas: true }),
    bunch({ n: 16, hue: 40, sat: 0.34, li: 0.58, len: 0.74, w: 0.018, lean: 0.86, seed: 0.20, heads: 2, pampas: true }),
    /* ---- ROW 3 : FORB / WHITE WILDFLOWERS ----------------------------- */
    rosette({ hue: 62, sat: 0.28, li: 0.42, len: 0.66, clumps: 4 }),
    (gg, r) => {
      rosette({ hue: 58, sat: 0.26, li: 0.44, len: 0.58, clumps: 4 })(gg, r);
      drawFlower(gg, T * 0.34, T, T * (0.58 + r() * 0.20), (r() - 0.5) * 0.28, 0,
        42, 0.08, 0.86, r);
      drawFlower(gg, T * 0.68, T, T * (0.50 + r() * 0.22), (r() - 0.5) * 0.30, 1,
        46, 0.10, 0.88, r);
    },
    (gg, r) => {
      rosette({ hue: 54, sat: 0.24, li: 0.40, len: 0.50, clumps: 3 })(gg, r);
      drawFlower(gg, T * 0.22, T, T * (0.62 + r() * 0.20), (r() - 0.5) * 0.28, 1,
        44, 0.08, 0.88, r);
      drawFlower(gg, T * 0.48, T, T * (0.70 + r() * 0.18), (r() - 0.5) * 0.26, 0,
        40, 0.07, 0.90, r);
      drawFlower(gg, T * 0.74, T, T * (0.58 + r() * 0.22), (r() - 0.5) * 0.32, 2,
        48, 0.10, 0.86, r);
    },
    /* Bloom-patch column. Grass.js wraps onto this tile inside a drift so
       white specks concentrate a few metres across, matching field_16. */
    (gg, r) => {
      rosette({ hue: 56, sat: 0.22, li: 0.40, len: 0.50, clumps: 3 })(gg, r);
      drawFlower(gg, T * 0.20, T, T * (0.68 + r() * 0.20), (r() - 0.5) * 0.28, 1,
        42, 0.08, 0.90, r);
      drawFlower(gg, T * 0.40, T, T * (0.58 + r() * 0.22), (r() - 0.5) * 0.30, 0,
        46, 0.06, 0.88, r);
      drawFlower(gg, T * 0.60, T, T * (0.72 + r() * 0.18), (r() - 0.5) * 0.24, 1,
        40, 0.09, 0.86, r);
      drawFlower(gg, T * 0.80, T, T * (0.64 + r() * 0.22), (r() - 0.5) * 0.32, 2,
        44, 0.07, 0.88, r);
    },
  ];
