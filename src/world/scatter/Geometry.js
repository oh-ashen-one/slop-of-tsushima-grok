...
/* ------------------------------------------- golden-field (non-western) */

/**
 * Walkable stone slab. Flat-ish top, chipped arrises, irregular plan — the
 * deck piece for a creek crossing and a mossy stepping stone on a bank.
 * Sits with its underside at y≈0 so `_seat` does not have to guess.
 */
export function makeStoneSlab(seed, { lod = 0 } = {}) {
  const rnd = streamRng(seed * 9137 + 5);
  const b = new Builder();
  const nx = lod === 0 ? 6 : 3;
  const nz = lod === 0 ? 4 : 2;
  const halfL = 0.86 + rnd() * 0.16;
  const halfW = 0.52 + rnd() * 0.12;
  const th = 0.15 + rnd() * 0.08;
  const dip = (rnd() - 0.5) * 0.045;
  const nSeed = (seed * 4283) | 0;
  ...
  for (let j = 0; j <= nz; j++) {
    for (let i = 0; i <= nx; i++) {
      const u = i / nx, v = j / nz;
      const wx = u * 2 - 1, wz = v * 2 - 1;
      const edge = Math.max(Math.abs(wx), Math.abs(wz));
      const wob = 1 + (noise3(wx * 2.1, 0.4, wz * 2.1, nSeed) * 0.07
        + (rnd() - 0.5) * 0.04) * edge;
      const x = wx * halfL * wob;
      const z = wz * halfW * wob;
      const grain = fbm3(x * 2.4, 0.35, z * 2.4, 2, nSeed) * 0.032;
      const yTop = th + dip * wx + grain;
      const yBot = fbm3(x * 1.6, -0.4, z * 1.6, 2, nSeed + 17) * 0.018;
      const cav = edge > 0.84 ? -0.45 : 0.12 + (1 - edge) * 0.18;
      top.push(b.vertex(x, yTop, z, 0, 1, 0, u, v, cav));
      bot.push(b.vertex(x, yBot, z, 0, -1, 0, u, 1 - v, 0.58));
    }
  }
  const col = nx + 1;
  for (let j = 0; j < nz; j++) {
    for (let i = 0; i < nx; i++) {
      const a = j * col + i;
      b.quad(top[a], top[a + 1], top[a + 1 + col], top[a + col]);
      b.quad(bot[a], bot[a + col], bot[a + 1 + col], bot[a + 1]);
    }
  }
  const loop = [];
  for (let i = 0; i <= nx; i++) loop.push(i);
  for (let j = 1; j <= nz; j++) loop.push(j * col + nx);
  for (let i = nx - 1; i >= 0; i--) loop.push(nz * col + i);
  for (let j = nz - 1; j >= 1; j--) loop.push(j * col);
  for (let k = 0; k < loop.length; k++) {
    const a = loop[k], c = loop[(k + 1) % loop.length];
    b.quad(bot[a], bot[c], top[c], top[a]);
  }
  b.smoothNormals();
  return b.toGeometry('stoneslab');
}

/**
 * Clump of tall thin canes. Midground verticals for a gold field — bamboo-like
 * sticks, not cactus. Instance scale of 1 is roughly a 2.5–4.5 m clump.
 */
export function makeBamboo(seed, { lod = 0 } = {}) {
  const rnd = streamRng(seed * 4517 + 19);
  const b = new Builder();
  const n = lod === 0 ? 7 + ((rnd() * 5) | 0) : 5;
  for (let i = 0; i < n; i++) {
    const a = rnd() * Math.PI * 2;
    const rad = rnd() * 0.30;
    const h = 2.1 + rnd() * 2.6;
    addLimb(b, {
      origin: new THREE.Vector3(Math.cos(a) * rad, -0.10, Math.sin(a) * rad),
      dir: new THREE.Vector3((rnd() - 0.5) * 0.16, 1, (rnd() - 0.5) * 0.16).normalize(),
      length: h,
      r0: 0.016 + rnd() * 0.014,
      r1: 0.009 + rnd() * 0.008,
      sides: lod === 0 ? 5 : 4,
      ...
      seed: seed * 31 + i,
      cav: 0.20,
      cap: { levels: 1, splinter: 0.08, tilt: 0.04, rings: 2 },
    });
  }
  b.smoothNormals();
  return b.toGeometry('bamboo');
}

/**
 * Torii-less stone marker: a standing stele with a companion cobble. A
 * compositional accent, not a shrine gate and not a mesa.
 */
export function makeStoneMarker(seed, { lod = 0 } = {}) {
  const rnd = streamRng(seed * 7727 + 41);
  const b = new Builder();
  const h = 1.42 + rnd() * 0.55;
  const halfW = 0.22 + rnd() * 0.10;
  const halfD = 0.09 + rnd() * 0.05;
  const nx = lod === 0 ? 4 : 2;
  const ny = lod === 0 ? 6 : 3;
  const taper = 0.78 + rnd() * 0.14;
  const lean = (rnd() - 0.5) * 0.08;
  const nSeed = (seed * 6151) | 0;
  ...
  for (let j = 0; j <= ny; j++) {
    ...
    const y = t * h;
    const k = 1 - (1 - taper) * t;
    const chip = t > 0.82 ? (1 - (t - 0.82) / 0.18) * (0.55 + rnd() * 0.5) : 1;
    for (let i = 0; i <= nx; i++) {
      const u = i / nx;
      const x = (u * 2 - 1) * halfW * k * (0.92 + (rnd() - 0.5) * 0.08) * chip;
      const zF = halfD * k * (0.90 + rnd() * 0.16);
      const zB = -halfD * k * (0.90 + rnd() * 0.16);
      const grain = fbm3(x * 3.1, y * 3.1, 0.2, 2, nSeed) * 0.012;
      const cav = (t > 0.9 || u < 0.08 || u > 0.92) ? -0.35 : 0.18;
      front.push(b.vertex(x + lean * y + grain, y, zF + grain, 0, 0, 1, u, t, cav));
      back.push(b.vertex(x + lean * y - grain, y, zB - grain, 0, 0, -1, u, t, cav));
    }
  }
  const col = nx + 1;
  for (let j = 0; j < ny; j++) {
    for (let i = 0; i < nx; i++) {
      const a = j * col + i;
      b.quad(front[a], front[a + 1], front[a + 1 + col], front[a + col]);
      b.quad(back[a], back[a + col], back[a + 1 + col], back[a + 1]);
    }
  }
  for (let j = 0; j < ny; j++) {
    const a = j * col, c = (j + 1) * col;
    const ar = j * col + nx, cr = (j + 1) * col + nx;
    b.quad(back[a], front[a], front[c], back[c]);
    b.quad(front[ar], back[ar], back[cr], front[cr]);
  }
  for (let i = 0; i < nx; i++) {
    const t0 = ny * col + i, t1 = ny * col + i + 1;
    b.quad(front[t0], front[t1], back[t1], back[t0]);
    b.quad(back[i], back[i + 1], front[i + 1], front[i]);
  }

  /* companion cobble at the foot — a marker is a pair, not a lone post */
  const ico = buildIcosphere(lod === 0 ? 2 : 1);
  const ox = halfW * 1.35, oz = halfD * 0.4;
  const rs = 0.16 + rnd() * 0.10;
  const start = b.vertexCount;
  for (const v of ico.verts) {
    const rr = rs * (0.85 + fbm3(v[0] * 2.4, v[1] * 2.4, v[2] * 2.4, 2, nSeed + 9) * 0.22);
    b.vertex(ox + v[0] * rr * 1.15, (v[1] + 0.55) * rr * 0.85, oz + v[2] * rr,
      v[0], v[1], v[2], v[0] * 0.5 + 0.5, v[1] * 0.5 + 0.5, 0.2);
  }
  for (const f of ico.faces) b.tri(start + f[0], start + f[1], start + f[2]);

  b.smoothNormals();
  return b.toGeometry('stonemarker');
}

/**
 * Tall thin pine trunk with self-pruned lower bole and short down-sloping
 * branches. Foliage is a separate card mesh (`makePineCrown`) so the two can
 * share wood / foliage materials.
 */
export function makePineTrunk(seed, { lod = 0 } = {}) {
  const rnd = streamRng(seed * 2203 + 7);
  const b = new Builder();
  const h = 7.4 + rnd() * 2.6;
  const r0 = 0.10 + rnd() * 0.05;
  const trunk = addLimb(b, {
    origin: new THREE.Vector3(0, -0.22, 0),
    dir: new THREE.Vector3((rnd() - 0.5) * 0.05, 1, (rnd() - 0.5) * 0.05).normalize(),
    length: h,
    r0,
    r1: r0 * 0.16,
    sides: lod === 0 ? 8 : 5,
  ...
  const nBr = lod === 0 ? 9 : 5;
  const segs = trunk.length - 1;
  for (let i = 0; i < nBr; i++) {
    const t = 0.36 + (i / nBr) * 0.58 + rnd() * 0.04;
    const ring = trunk[Math.min(segs, Math.round(t * segs))];
    const a = i * 2.39996 + rnd() * 0.35;
    const dir = new THREE.Vector3(Math.cos(a), -0.22 - rnd() * 0.38, Math.sin(a)).normalize();
    const len = (0.50 + rnd() * 0.75) * (1.2 - t);
    addLimb(b, {
      origin: ring.pos.clone(),
      dir,
      length: len,
  ...
  b.smoothNormals();
  return b.toGeometry('pinetrunk');
}

/**
 * Conical needle-plate cards. UVs pick a cell of the bush atlas; instance tint
 * pulls them to near-black pine green against gold grass.
 */
export function makePineCrown(seed, { lod = 0, atlas = 2 } = {}) {
  const rnd = streamRng(seed * 3343 + 11);
  const b = new Builder();
  const cards = lod === 0 ? 20 : 9;
  for (let i = 0; i < cards; i++) {
    const t = i / Math.max(1, cards - 1);
    const y = 0.34 + t * 0.90;
    const rad = (1 - t * 0.82) * (0.40 + rnd() * 0.12);
    const yaw = t * Math.PI * 2 * 2.39996 + rnd() * 0.55;
    const cell = (rnd() * atlas * atlas) | 0;
    const cu = (cell % atlas) / atlas;
    const cv = ((cell / atlas) | 0) / atlas;
    const s = 1 / atlas;
    const w = (0.38 + rnd() * 0.28) * (1.05 - t * 0.35);
    const hh = (0.32 + rnd() * 0.22) * (1.05 - t * 0.25);
    const ox = Math.cos(yaw) * rad;
    const oz = Math.sin(yaw) * rad;
    const cy = Math.cos(yaw), sy = Math.sin(yaw);
    const tilt = -0.35 - rnd() * 0.25;
    const ct = Math.cos(tilt), st = Math.sin(tilt);
    const corner = (sx, syy) => {
      const lx = sx * w;
      const ly = syy * hh;
      const x = ox + (lx * ct - ly * st) * cy;
      const yy = y + (lx * st + ly * ct);
      const z = oz + (lx * ct - ly * st) * sy;
      return b.vertex(x, yy, z, -sy, 0.42, cy,
        cu + (sx * 0.5 + 0.5) * s, cv + (syy * 0.5 + 0.5) * s, 0.18);
    };
    const a = corner(-1, -1), b2 = corner(1, -1), c = corner(1, 1), d = corner(-1, 1);
    b.quad(a, b2, c, d);
  }
  const n = b.vertexCount;
  for (let i = 0; i < n; i++) {
    const x = b.pos[i * 3], y = b.pos[i * 3 + 1] - 0.4, z = b.pos[i * 3 + 2];
    const l = Math.hypot(x, y, z) || 1;
    b.nrm[i * 3] = (x / l) * 0.70;
    b.nrm[i * 3 + 1] = (y / l) * 0.45 + 0.55;
    b.nrm[i * 3 + 2] = (z / l) * 0.70;
    const ll = Math.hypot(b.nrm[i * 3], b.nrm[i * 3 + 1], b.nrm[i * 3 + 2]) || 1;
    b.nrm[i * 3] /= ll; b.nrm[i * 3 + 1] /= ll; b.nrm[i * 3 + 2] /= ll;
  }
  return b.toGeometry('pinecrown');
}
