 ...
 * layer is re-tinted to a target mid-tone from the Iki Island gold-field
 * palette: warm sunlit grass, cream-straw dry grass, dark warm brown soil.
 * Per-texel variation is preserved — only the mean is moved.
 *
 * The two grass tints sit at ~0.50 saturation with hues of ~49deg and ~44deg —
 * gold and straw, not sage, not lawn. Slightly brighter than the old western
 * pair so the ground reads as a field even without grass cards.
 */
const LAYER_TINT = [
  [158, 142, 74],    // grass_prairie — warm gold-green, hue ~49deg
  [188, 162, 88],    // grass_dry     — sunlit straw-gold, hue ~44deg
  [102, 80, 50],     // dirt_dry      — dark warm brown (#665032)
  [118, 100, 82],    // rock_cliff    — warm rock, not blue-grey
  [116, 104, 86],    // scree         — warm gravel
  [156, 140, 102],   // sand_fine     — tan, not bleached grey
  [222, 226, 234],   // snow
];
...
void terrainSurface() {
  ...
  /* Gold vs straw across the field, driven by the macro mask rather than one
     flat green. Total grass is preserved, so world.getSurface() still agrees. */
  float gTot = w[0] + w[1];
  if (gTot > 0.001) {
    /* Pulled toward straw-gold before the noise is added: if the baked ratio is
    ...
    float dryF = clamp(mix(w[1] / gTot, 0.55, 0.45)
  ...
  float bleach = arid * 0.28 * (1.0 - snowW) * (1.0 - grassW * 0.72);
...
export function makeTerrainMaterial(uniforms, gridN) {
  ...
  mat.customProgramCacheKey = () => 'terrain-cdlod-v4' + (dbg ? '-dbg' : '');
  return mat;
}
