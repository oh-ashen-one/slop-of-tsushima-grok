...
export function bakeSplat(field, h, acc, wet, res, cellSize, waterLevel, half = 4096,
  ...
  for (let y = 0; y < res; y++) {
    ...
    for (let x = 0; x < res; x++) {
      ...
      const field = clamp(wp * 1.20 + wv * 0.55 + wf * 0.18, 0, 1);

      /* --- rock: steep ground, hard strata, high mountains. Plains keep soil. */
      const steep = smoothstep(0.50, 0.95, slope);          // 27deg .. 44deg
      let rock = steep * (0.62 + hard * 0.6);
      /* Massif faces are bedrock from about 15 degrees up — there is nowhere
         for soil to stay on a retreating scarp. */
      rock += smoothstep(0.30, 0.66, slope) * wm * 1.05;
      rock += smoothstep(0.34, 0.70, slope) * wf * 0.6;
      rock += smoothstep(0.40, 0.78, slope) * wb * 0.35;
      ...
      rock += clamp(0.5 - concave, 0, 0.5) * smoothstep(0.34, 0.68, slope) * 1.15
        * (1 - field * 0.85);
      /* rare bald outcrop — not a peppering of rock across the grassland */
      rock += smoothstep(0.94, 0.995, fbm01(wx * 1.7 - 900, wz * 1.7 + 300, 3, 1 / 220))
        * (1 - smoothstep(0.30, 0.55, slope)) * 0.10 * (1 - field);
      rock *= mix(1, 0.10, field * (1 - steep));
      rock = clamp(rock, 0, 1);
      ...
      scree += smoothstep(0.30, 0.62, slope) * smoothstep(0.44, 0.88, concave)
        * (wm + wb * 0.25) * 0.95;
      scree += midSlope * smoothstep(0.55, 0.95, concave) * hard * 0.5 * (wm + wf);
      /* sparse gravel only on genuinely arid ground, never the gold field */
      scree += smoothstep(0.70, 0.96, fbm01(wx * 1.3 + 5100, wz * 1.3 - 2200, 3, 1 / 190))
        * smoothstep(0.62, 0.90, arid) * (1 - smoothstep(0.16, 0.38, slope))
        * 0.16 * (1 - field);
      scree *= mix(1, 0.08, field * (1 - steep));
      scree = clamp(scree, 0, 1);

      /* --- dirt: gullies, washes, banks — not the hero surface of the basin */
      let dirt = smoothstep(0.14, 0.40, slope) * 0.32
        + flow01 * 0.50
        + concave * 0.14 * (1 - steep)
        + wb * 0.10 + wv * 0.10
        + smoothstep(0.78, 0.96, patchB) * 0.14;
      dirt *= (0.32 + arid * 0.70);
      dirt *= mix(1, 0.28, field);
      dirt = clamp(dirt, 0, 1);

      /* --- sand: dune region and far pans, not the valley floor */
      let sand = (ws * 1.05 + smoothstep(0.72, 0.95, arid) * 0.18)
        * (1 - smoothstep(0.10, 0.30, slope))
        * (1 - smoothstep(140, 300, altJ));
      sand += smoothstep(0.7, 1.0, flow01) * 0.22 * (1 - smoothstep(0.12, 0.3, slope));
      sand *= mix(1, 0.12, field);
      sand = clamp(sand, 0, 1);

      /* --- grass: flats, moisture, the plains/valley default */
      const flat = 1 - smoothstep(0.14, 0.52, slope);
      const moist = clamp(1 - arid * 0.80 + flow01 * 0.55 + wv * 0.50 + wp * 0.42
        + (1 - smoothstep(waterLevel + 4, waterLevel + 70, alt)) * 0.3, 0, 1.5);
      const grassTotal = clamp(
        flat * (0.50 + moist * 0.95) * (1 - rock * 0.90) + wp * flat * 0.38,
        0, 1);
      const dryness = clamp(arid * 0.85 + smoothstep(160, 420, altJ) * 0.28
        - flow01 * 0.55 - wv * 0.35 - wp * 0.18 + (patchA - 0.5) * 0.45, 0, 1);
