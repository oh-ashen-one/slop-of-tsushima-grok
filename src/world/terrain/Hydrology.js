import { STREAM_PTS } from './Field.js';
import { polylineDist, polylineMetrics, smoothstep, mix } from './Noise.js';
...
export function flowAccumulate(filled, res, cellSize) {
  ...
  /* Authored grassland creek: guarantee catchment along STREAM_PTS so the
     Water system and isWater both see a channel even if the MFD network
     wandered off the cut. Tuned so the crossing is ~8–14 m of wet. */
  const half = res * cellSize * 0.5;
  for (let y = 0; y < res; y++) {
    const z = -half + (y + 0.5) * cellSize;
    for (let x = 0; x < res; x++) {
      const wx = -half + (x + 0.5) * cellSize;
      const sv = polylineDist(wx, z, STREAM_PTS, STREAM_M.cum, STREAM_M.total);
      const halfW = mix(5.5, 12.0, Math.pow(sv.t, 0.55));
      if (sv.d > halfW * 1.7) continue;
      const axis = smoothstep(halfW * 1.20, halfW * 0.18, sv.d);
      if (axis < 0.02) continue;
      const want = mix(1.7e5, 4.2e5, Math.pow(sv.t, 0.9)) * axis;
      const i = y * res + x;
      if (acc[i] < want) acc[i] = want;
    }
  }
  return { acc, recv };
}
