...
import {
  fbm2, smooth, padMicro, padRelief, TOWN_PLAN,
} from './Pad.js';
...
export function buildPad(B, mat, street, o) {
  ...
  for (let j = 0; j <= nT; j++) {
    ...
    for (let i = 0; i <= nS; i++) {
      ...
      /* --- surface colouring: gravel courtyard, not a wagon street ------ */
      const at = Math.abs(t);
      const court = 1 - smooth(9.5, 16.0, at);
      const path = 1 - smooth(1.6, 3.4, at);
      const dust = fbm2(p[0] * 0.07, p[1] * 0.07);
      const fine = fbm2(p[0] * 0.55, p[1] * 0.55);
      const chip = fbm2(p[0] * 1.8 + 9.1, p[1] * 1.8);
      /* Pale warm gravel in the precinct, packed slightly darker down the
       * sandō, earth at the rim. Speckle from `chip` is the grain that stops
       * the pad reading as a painted plane at 16:30. */
      let k = 1.10 + 0.22 * (dust - 0.5) + 0.10 * (fine - 0.5) + 0.08 * (chip - 0.5);
      k *= 1 - court * 0.04;
      k *= 1 - path * 0.10;
      const moss = Math.max(0, (fbm2(p[0] * 0.11 + 4.2, p[1] * 0.11) - 0.62) * 2.4) * court;
      k *= 1 - moss * 0.08;
      C[j * row + i] = [
        k * (0.96 - 0.04 * path + 0.03 * (1 - court)),
        k * (0.90 - 0.02 * path + moss * 0.06),
        k * (0.78 - 0.04 * path + moss * 0.03) * (1 - f * 0.02),
      ];
    }
  }
