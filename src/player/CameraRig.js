...
/**
 * GOLDEN FIELD — CAMERA RIG
 * ============================================================================
 * Ghost of Tsushima third person: low, wide, looking THROUGH the grass.
 *
 *   SPRING ARM   pivot at chest height (not hat-brim), arm close behind and
 *                nearly centred. Critically-damped follow on pivot and orbit.
...
export class CameraRig {
  ...
  constructor(ctx) {
    ...
    // Tsushima: intimate on foot (~3.75 m), longer in the saddle so the horse
    // and the gold field both read. Speed still lengthens from these bases.
    this.armLength = 3.75;
    this._arm = 3.75;
    this._armBlocked = 3.75;
    this.shoulder = 0.42;
    this.height = 1.18;
    this.mountHeight = 1.44;
    this.baseFov = 56;
    this._fov = 56;
    this._yaw = 0;
    this._pitch = -0.16;
  ...
  update(dt) {
    ...
    // Mounted, `p` is the horse's FEET. Stock 2.10 looked down from the
    // rider's chest (RDR2 hat-brim). 1.44 sits just above a working saddle,
    // well below cowboy-hat height, so the lens is in the grass and the
    // rider silhouettes against the field with the horizon in the upper third.
    const targetHeight = lerp(this.height, this.mountHeight)
      - (player ? player.crouch * 0.35 * (1 - mounted) : 0)
      + sk * lerp(0.10, 0.16)                  // stay in the grass even at speed
      + tK * 0.10                              // and a touch more mid-transition
      // ADS still lifts to the shooter's eyeline so the brim does not eat the
      // barrel, but the rest pose stays at the chest, not the hat.
      + aim * 0.20;
    ...
    if (!this._init) {
      this.pivot.copy(_v); this._yaw = yawIn;
      this._pitch = pitchIn + lerp(-0.10, -0.04);
    ...
    this._yaw += step;
    // On foot: a few extra degrees down so waist-high grass fills the lower
    // third. Mounted: look ahead toward the horizon (Iki Island ride), easing
    // the gaze UP a little at a gallop instead of burying it in the dirt.
    const pitchBias = lerp(-0.10, -0.04);
    const pitchT = pitchIn + pitchBias - sk * lerp(0.05, -0.03);
     ...
     */
    // Intimate on foot, long enough mounted that the horse and the gold field
    // both read. Speed still lengthens from those bases; stand off a little
    // further still while the rider is climbing on or off.
    this.armLength = (lerp(3.75, 5.60) + sk * lerp(1.35, 1.55) + tK * 0.70)
      * (1 - aim * 0.52);
    const fovT = (this.baseFov + sk * lerp(5.0, 7.0) + tK * 3.5) - aim * 10.5;
