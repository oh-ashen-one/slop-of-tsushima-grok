...
import { RoninHero } from './RoninHero.js';
...
export class Player {
  ...
  constructor(ctx) {
    ...
    this.drawCalls = 0;
    /** Staged GLB hero when `/assets/ronin.glb` loaded; otherwise null. */
    this.hero = null;
    this._cowboy = null;
  }
  ...
  /**
   * Swap the procedural cowboy for the staged ronin when the GLB is present.
   * Failure must not break boot — HumanRig stays the visible fallback.
   */
  async _tryRoninHero() {
    const ctx = this.ctx;
    try {
      const hero = new RoninHero();
      await hero.init(ctx);
      this.hero = hero;
      this.group.add(hero.root);
      if (this._cowboy) {
        this._cowboy.visible = false;
        this._cowboy.castShadow = false;
      }
      this.mesh = hero.mesh || hero.root;
      this.state.height = 1.78;
      const lighting = ctx.get('lighting');
      if (lighting && lighting.requestShadowCaster) lighting.requestShadowCaster(hero.root);
      this._hideRifle();
    } catch {
      this.hero = null;
    }
  ...
  _hideRifle() {
    ...
    if (W.meshWood) W.meshWood.visible = false;
    if (W.meshSteel) W.meshSteel.visible = false;
    if (W.meshLever) W.meshLever.visible = false;
  }

  /**
   * Mixamo Hips sit ~0.9 m above the feet. The saddle bone is the seat, and
   * SEAT_RISE is the hip joint above that bone, so the visual has to drop by
   * hipY − SEAT_RISE rather than the procedural PROP.hipY 0.92.
   */
  _heroSeatOffset() {
    const hipY = this.hero ? this.hero.getHipY() : 0.90;
    const k = this.mountBlend || 0;
    return {
      x: 0,
      y: (SEAT_RISE - hipY) * k,
      z: SEAT_BACK * k,
  ...
  _driveHero(dt) {
    if (!this.hero) return;
    const seat = this._heroSeatOffset();
    this.hero.root.position.set(seat.x, seat.y, seat.z);
    this.hero.update(dt || 0, {
      mode: this.mode,
      speed01: this.speed01,
      mounting: this.mode === 'mounting',
    });
    this._hideRifle();
  }
  ...
  _writeCtx() {
    ...
    p.height = this.hero ? 1.78 : PROP.height;
  ...
  _poseHeroOnFoot(dt) {
    ...
    const yaw = this.bodyYaw;
    const base = this.renderPos;
    const stanceY = world.getHeight(base.x, base.z);
    this.group.position.set(base.x, stanceY, base.z);
    this.group.rotation.set(0, yaw, 0);
    ...
    this.mountBlend = 0;
    this._driveHero(dt);
    if (this.weapon) this.weapon.update(dt || 1 / 60);
    if (this.contact) {
      const gy = stanceY;
      _v.set(base.x, gy, base.z);
      _v2.set(base.x, gy, base.z);
      this.contact.update([_v, _v2, _v], [0.32, 0.32, 0.62], [0.85, 0.85, 0.55]);
    }
    this._updateVisibility();
  }
  ...
  _updateVisibility() {
    ...
    if (this.hero) {
      this.hero.setVisible(vis);
      if (this._cowboy) this._cowboy.visible = false;
      this._hideRifle();
    } else if (this.weapon && this.weapon.group) {
      this.weapon.group.visible = vis;
    }
  ...
  _poseTransition(dt) {
    ...
    if (useRig) {
      ...
      this._driveHero(dt);
      if (this.weapon) this.weapon.update(dt || 1 / 60);
      ankles.push(_stand.clone(), _stand.clone());
    }
  ...
  dispose() {
    ...
    if (this._cowboy && this._cowboy.geometry) this._cowboy.geometry.dispose();
    else if (this.mesh && this.mesh.geometry) this.mesh.geometry.dispose();
