...
/**
 * ============================================================================
 *  HUD — ink and gold leaf, as little as possible.
 * ============================================================================
 *
 * Everything lives on one 2D canvas overlay drawn at true device resolution:
 * a thin compass that surfaces while you are moving, two condition arcs that
 * only exist while they are not full, a contextual prompt (E / 馬), and a
 * location title card. Gold ink on nothing — the field is the picture.
 *
 * Wanted stars, ammo ticks, pelts, the rifle reticle and "the law" are not
 * drawn. Other systems may still publish that state; this overlay ignores it.
 *
...
const FONT = '"Hiragino Mincho ProN", "Hiragino Mincho Pro", "Yu Mincho", "YuMincho", "MS PMincho", "Songti SC", "Times New Roman", Georgia, serif';

/* Gold leaf on near-black. #d4b45c and a quieter cream. */
const INK = [232, 220, 186];
const INK_DIM = [196, 176, 130];
const GOLD = [212, 180, 92];
...
export class HUD {
  ...
  notify(text) {
    const t = String(text);
    /* Other systems still shout about bounties, pelts, the law. Swallow it. */
    if (/wanted|the law|lawmen|skinned|stowed|pelt|bounty/i.test(t)) return;
    this._notice = t;
    this._noticeT = 2.6;
  }
  ...
  update(dt) {
    ...
    const target = {
      ...
      /* Rifle chrome, pelts, wanted stars: never drawn. */
      weapon: 0,
      reticle: 0,
      wanted: 0,
      skin: 0,
  ...
  _scanInteractables() {
    ...
    /* Hide pelt / carcass verbs — hunting chrome. */
    if (pl && (pl.pickup || pl.carcass)) { this._promptT = 0; return; }
    if (ctx.player.mode === 'mounted') {
      ...
      if ((ctx.player.speed01 || 0) < 0.06) best = { key: 'E', label: 'Dismount  馬' };
    } else {
      const horse = ctx.get('horse');
      if (horse && horse.state) consider(horse.state.position, 'E', 'Mount  馬', MOUNT_RANGE);
      const fire = ctx.poi.get('camp_fire');
      if (fire) consider(fire.pos || fire, 'E', 'Rest', 4.2);
    }
  ...
  _draw() {
    ...
    const botA = Math.max(
      ...
      this.a.cores * 0.45, this.a.hint * 0.7,
    );
    if (botA > 0.004) this._scrimBottom(c, W, H, s, 0.44 * botA * G);
    const topA = this.a.compass * 0.22;
    ...
    if (this.a.compass > 0.004) this._drawCompass(c, W, H, s, this.a.compass * G * 0.62);
    ...
    if (this.a.prompt > 0.004) this._drawPrompt(c, W, H, s, this.a.prompt * G);
  ...
  _drawControls(c, W, H, s, A) {
    ...
    const rows = mounted
      ? [
        [['W', 'RIDE', 'ride'], ['SHIFT', 'RUN', 'spur'],
          ['A D', 'TURN', 'rein'], ['E', '馬', 'mount']],
      ]
      : [
        [['W A S D', 'WALK', 'move'], ['SHIFT', 'RUN', 'run'],
          ['E', 'MOUNT  馬', 'mount']],
      ];
  ...
  _drawCompass(c, W, H, s, A) {
    const width = Math.min(W * 0.26, 320 * s);
    const cx = W * 0.5;
    const cy = Math.round(36 * s);
    const span = 100;                       // degrees visible across the strip
     ...
     */
    const start = Math.ceil((head - span * 0.5) / 30) * 30;
    for (let d = start; d <= head + span * 0.5; d += 30) {
      ...
      const inter = dd % 45 === 0;
      const len = cardinal ? 8 * s : inter ? 5 * s : 3 * s;
      const al = (cardinal ? 0.92 : inter ? 0.5 : 0.28) * f * A;
      this._hairline(c, x, Math.round(cy - len), x, Math.round(cy) - 1,
        cardinal ? GOLD : INK, al, cardinal ? Math.max(1, 1.15 * s) : Math.max(0.7, 0.85 * s));
      ...
      if ((cardinal || inter) && f > 0.28) {
        ...
        this._text(c, label, xr, cy - len - 5 * s, {
          size: (cardinal ? 11 : 8.5) * s,
          track: 0.08,
          align: 'center',
          alpha: (cardinal ? 0.88 : 0.5) * f * A,
          colour: cardinal ? GOLD : INK_DIM,
          shadow: 0.85,
        });
      }
    }
    ...
    c.beginPath();
    c.moveTo(cx, cy + 4 * s);
    c.lineTo(cx - 2.4 * s, cy + 0.5);
    c.lineTo(cx + 2.4 * s, cy + 0.5);
    c.closePath();
    c.strokeStyle = `rgba(7,6,5,${0.4 * A})`;
    c.lineWidth = 1.4;
    c.stroke();
    c.fillStyle = rgba(GOLD, 0.88 * A);
    c.fill();
  }
  ...
  _drawPrompt(c, W, H, s, A) {
    ...
    const label = displayLabel(this._prompt.label);
  ...
  _drawTitle(c, W, H, s, A) {
    ...
    const name = this._title.name;
    const cjk = hasCJK(name);
    this._text(c, cjk ? name : name.toUpperCase(), x, y + rise, {
      size: (cjk ? 26 : 22) * s, track: cjk ? 0.14 : 0.28, alpha: 0.92 * A,
      colour: INK, shadow: 1.0, outline: 0.4,
    });
    /*
  ...
  _drawNotice(c, W, H, s, A) {
    ...
    const note = displayLabel(this._notice);
    this._text(c, note, W * 0.5, y, {
      size: 12 * s, track: hasCJK(note) ? 0.08 : 0.14, align: 'center',
      alpha: 0.82 * A, colour: INK, shadow: 0.95,
    });
  }
...
const LEARN = 2.6;
const FOOT_TAGS = ['move', 'run', 'mount'];
const MOUNTED_TAGS = ['ride', 'spur', 'rein', 'mount'];

const CJK = /[\u3040-\u30ff\u3400-\u9fff]/;
function hasCJK(str) { return CJK.test(str || ''); }
function displayLabel(str) {
  return String(str || '').replace(/[A-Za-z]+/g, (m) => m.toUpperCase());
}
