...
export const RECIPES = {
  ...
  dirt_dry: {
    ...
    albedo: (h, x, y, s) => {
      let c = mix3([132, 104, 70], [156, 124, 84], clamp01(s.m * 1.3 - 0.15));
      /* pebble lag: warm stone, not cool blue-grey */
      c = mix3(c, [120, 102, 76], clamp01((s.b1 - 0.62) * 2.6) * 0.7);
      const t = 0.70 + h * 0.44 + speckle(x, y, 170, 47) * 0.13;
      return [c[0] * t, c[1] * t, c[2] * t];
    },
    wear: { cavity: 0.58, cavityCol: [64, 48, 32], edge: 0.42, edgeCol: BLEACH, cavRough: 0.06 },
  },
  ...
  dirt_packed: {
    ...
    albedo: (h, x, y, s) => {
      /* dust between the stones, each stone a warm mineral brown */
      let c = mix3([122, 96, 66], [150, 120, 84], clamp01(s.b0 * 1.5 - 0.25));
      c = mix3(c, mix3([108, 90, 68], [138, 114, 84], s.m), clamp01(s.m * 2.2));
      const t = 0.68 + h * 0.48 + speckle(x, y, 175, 61) * 0.14;
      return [c[0] * t, c[1] * t, c[2] * t];
    },
    wear: { cavity: 0.62, cavityCol: [58, 44, 30], edge: 0.38, edgeCol: [190, 170, 136], cavRough: 0.05 },
  },
  ...
  mud: {
    ...
    albedo: (h) => [68 * h + 28, 50 * h + 18, 32 * h + 12],
    wear: { cavity: 0.8, cavityCol: [24, 16, 10], edge: 0.24, edgeCol: [128, 102, 72], cavRough: -0.16 },
  },

  /*
   * Tsushima gold field, NOT sage-grey prairie and NOT a lawn.
   * Olive-gold (hue ~71deg, sat ~0.31) mixed with sunlit gold (hue ~50deg,
   * sat ~0.35), cream pampas tips, warm soil in bald patches. Sat stays
   * moderate so grass_not_emerald (≤0.34) holds; emerald ~120deg is banned.
   */
  grass_prairie: {
    ...
    albedo: (h, x, y, s) => {
      ...
      const tip = clamp01((h - 0.56) * 2.4 + s.cvx * 0.22) * 0.40;
      const v = 0.76 + h * 0.50 + (s.b1 - 0.5) * 0.18;
      let c = mix3([134, 142, 98], [176, 166, 114], dry);
      c = mix3(c, [110, 88, 58], soil);
      c = mix3(c, [243, 232, 200], tip);
      return [c[0] * v, c[1] * v, c[2] * v];
    },
    wear: { cavity: 0.44, cavityCol: [46, 52, 28], edge: 0.46, edgeCol: [243, 232, 200], cavRough: 0.04 },
  },
  ...
  grass_dry: {
    ...
    albedo: (h, x, y, s) => {
      ...
      const tip = clamp01((h - 0.54) * 2.2 + s.cvx * 0.20) * 0.36;
      const v = 0.78 + h * 0.46 + (s.b1 - 0.5) * 0.20;
      let c = mix3([156, 146, 102], [184, 170, 118], s.m);
      c = mix3(c, [118, 94, 62], soil);
      c = mix3(c, [243, 232, 200], tip);
      return [c[0] * v, c[1] * v, c[2] * v];
    },
    wear: { cavity: 0.46, cavityCol: [52, 46, 28], edge: 0.48, edgeCol: [243, 232, 200], cavRough: 0.04 },
  },
  ...
  sand_fine: {
    ...
    albedo: (h, x, y) => {
      ...
      return [140 * t, 128 * t, 98 * t];
    },
    wear: { cavity: 0.34, cavityCol: [78, 64, 44], edge: 0.36, edgeCol: BLEACH, cavRough: 0.03 },
  },
  ...
  bark_pine: {
    ...
    albedo: (h, x, y, s) => {
      /* near-black dark brown plates — silhouette trunks against gold grass */
      let c = mix3([36, 28, 20], [52, 40, 28], s.m);
      c = mix3(c, [22, 16, 12], clamp01((0.44 - h) * 2.4) * 0.88);
      const t = 0.56 + h * 0.42 + speckle(x, y, 150, 311) * 0.10;
      return [c[0] * t, c[1] * t, c[2] * t];
    },
    wear: { cavity: 0.72, cavityCol: [16, 12, 8], edge: 0.38, edgeCol: [72, 60, 44], cavRough: 0.06 },
  },
  ...
  leaves_atlas: {
    ...
    albedo: (h, x, y) => {
      const autumn = clamp01(vfbm(x, y, { octaves: 2, freq: 6, seed: 407 }) * 1.35 - 0.58);
      let c = mix3([90, 96, 64], [140, 132, 84], h);
      c = mix3(c, [196, 90, 40], autumn * 0.38);
      return c;
    },
  },

  /* olive-gold scrub with sparse autumn — not grey sagebrush */
  foliage_scrub: {
    ...
    albedo: (h, x, y) => {
      const autumn = clamp01(vfbm(x, y, { octaves: 2, freq: 5, seed: 427 }) * 1.3 - 0.62);
      let c = mix3([92, 100, 66], [146, 136, 88], h);
      c = mix3(c, [184, 96, 44], autumn * 0.32);
      return c;
    },
  },
