 ...
 *
 *  1. FIELDS (rain, snow, motes/pollen, petals) are stateless: a particle's
 *     position is an analytic function of its seed and the clock, wrapped into
 *     a box that follows the camera. The CPU never touches a vertex, the whole
 *     field is "full" the instant weather changes (no warm-up — which matters,
 *     the capture harness only allows 2.5 s of settle), and 8000 raindrops cost
 *     one draw call and no bandwidth. Petals auto-run in clear/fair/golden-hour
 *     so capture frames already have the Tsushima drift without an emitter().
 *
 ...
 *         petals breath mist sparks blood
 */
export class Particles {
  ...
  async init() {
    ...
    /* Sakura + maple: one cheap field, always full. Count stays a few dozen
       to a few hundred in frame even at the default 6k budget. */
    this.petals = this._makeField('petals', Math.max(140, Math.round(560 * s)), 'MODE_PETAL', {
      box: new THREE.Vector3(20, 12, 20),
      yOffset: 3.0,
      size: 0.125, stretch: 1, aspect: 0.74, fall: 0.40,
      tile: [0.5, 0.5, 0.5, 0.0],
      tint: new THREE.Color(0.97, 0.90, 0.78),
      opacity: 0.0, emissive: 0.08,
      blending: THREE.NormalBlending,
      softness: 0.55, phaseGain: 1.2,
    });
  ...
  _makeField(name, count, mode, o) {
    ...
    if (o.softness != null) uniforms.uSoftness = { value: o.softness };
    if (o.phaseGain != null) uniforms.uPhaseGain = { value: o.phaseGain };
